...
describe("Denial of Service", () => {
    it("After being declared the winner, AttackDOS.sol should not allow anyone else to become the winner", async () => {
        // Deploy the GoodDOS Contract
        const goodContract = await hre.ethers.deployContract("GoodDOS", []);
        await goodContract.waitForDeployment();

        // Deploy the AttackDOS Contract
        const attackContract = await hre.ethers.deployContract("AttackDOS", [
            goodContract.target,
        ]);

        await attackContract.waitForDeployment();

        // Now let's attack the GoodDOS Contract
        ...
        // Start the attack and make AttackDOS.sol the current winner of the auction
